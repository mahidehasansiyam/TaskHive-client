...
const SubmitProposal = ({ task, isFreelancer,session }) => {
 ...
 const handleProposalSubmit = async e => {
   ...
   try {
     const response = await fetch('http://localhost:9000/proposals', {
       method: 'POST',
       headers: {
            ...
            {isFreelancer && (
              <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                ...
                <form onSubmit={handleProposalSubmit} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    className="w-full py-3 bg-gradient-to-r from-[#f59e0b] to-[#ea580c] rounded-lg text-black  font-semibold "
                  >
                    Submit Proposal
                  </button>
                </form>
